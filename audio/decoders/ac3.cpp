...
public:
	...
	uint getChannels() const { return _audStream->getChannels(); }
...
void AC3Stream::queuePacket(Common::SeekableReadStream *data) {
	...
	while (packet->pos() < packet->size()) {
		...
		if (_frameSize == 0) {
			...
			if (outputLength > 0)
				_audStream->queueBuffer((byte *)outputBuffer, outputLength, DisposeAfterUse::YES, FLAG_STEREO | FLAG_16BITS | FLAG_NATIVE_ENDIAN);
