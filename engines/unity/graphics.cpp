...
void Graphics::playMovie(Common::String filename) {
	// Initialize graphics to true color mode
	initGraphics(640, 480, true, 0);
	if (g_system->getScreenFormat().bytesPerPixel == 1)
		error("Failed to switch to true color mode");
	...
	// Switch to the video size (FIXME: This sucks)
	initGraphics(vidwidth, vidheight, false, 0);
	if (g_system->getScreenFormat().bytesPerPixel == 1)
		error("Failed to switch to true color mode");
	...
	while (!g_engine->shouldQuit() && !videoDecoder->endOfVideo() && !skipVideo) {
		if (videoDecoder->needsUpdate()) {
			...
			if (frame) {
				...
				const ::Graphics::Surface *drawFrame = frame;

				if (frame->format != g_system->getScreenFormat()) {
					convertedFrame = frame->convertTo(g_system->getScreenFormat());
					drawFrame = convertedFrame;
				}

				g_system->copyRectToScreen((const byte *)drawFrame->getPixels(), drawFrame->pitch,
					0, 0, drawFrame->w, drawFrame->h);
